...
async function handleCreateMockup(msg) {
    ...
    try {
        await createDeviceMockup(validationResult.selectedFrame, msg.model);
        figma.ui.postMessage({
            type: 'success',
...
function isValidDeviceModel(model) {
    return ['iphone-14-pro', 'iphone-14', 'iphone-se', 'android-medium'].includes(model);
}
async function createDeviceMockup(selectedFrame, modelKey) {
...
async function createDeviceFrameOverlay(parentFrame, modelKey, model) {
    try {
